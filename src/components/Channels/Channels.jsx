...
import {
  Col, Dropdown, Nav, NavItem, NavLink, Button, ButtonGroup,
    ...
    <Col className="col-3 border-right">
      ...
      <Nav variant="pills" className="flex-column" activeKey={currentChannelId} onSelect={handleSelectChannel}>
        {channels.map((channel) => {
          ...
          return (
            <NavItem key={id}>
              <NavLink eventKey={id} className={classnames('mb-2 text-left d-flex justify-content-between', btnTypeClass)}>
                <span>{name}</span>
                {removable && (
                <Dropdown as={ButtonGroup}>
                  <Dropdown.Toggle id={id} className={classnames(btnTypeClass, 'pl-1 pr-1 pt-0 pb-0')} />
